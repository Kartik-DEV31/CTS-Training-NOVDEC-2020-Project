package managmentApplication.applicationRepository;
...
public interface SpecificRestaurantRepository extends JpaRepository<SpecificRestaurantDetails, Integer> {
	...
	@Query("select s from SpecificRestaurantDetails s WHERE s.id = ?1")
	List<SpecificRestaurantDetails> findByOwnerId(Integer ownerId);

	@Query("select SUM(s.employeeCount) from SpecificRestaurantDetails s WHERE s.id = ?1")
